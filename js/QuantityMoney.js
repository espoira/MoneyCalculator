	...
	slider.onmouseup = function() {		
		...
		if (S_InMonth==0) {
			...
			function MoneyMoving(moneyPic,i,k,n){
				...
				var toTop = String(-Math.floor((i-1)/2)*60);
			...
			for (var i = 1; i <= kNP; i++){
				...
				document.getElementById('pic-1').appendChild(moneyPic[i]);
			};
			
			var kDep = MoneyDeposit(S_InMonth);
			for (var i = 11; i <= kDep+10; i++){
				if (kDep<=5) {k = 30;
						if (kDep<=2) {n = 45;} else n = (10-kDep)*5;
					} 
				else { k = 25; 
					   if (kDep<=8) {n = 18;} else n = 5;
					 };
				MoneyMoving(moneyPic[i],i-10,k,n);
				document.getElementById('pic-2').appendChild(moneyPic[i]);
			};
			
			var kInv = MoneyInvest(S_InMonth);
			for (var i = 21; i <= kInv+20; i++){ 
				if (kInv<=5) {k = 30;
						if (kInv<=2) {n = 70;} else n = 55;
					} 
				else { k = 25; 
					   if (kInv<=8) {n = 35;} else n = 30;
					 };
				if (kInv==5) n = 45;
				MoneyMoving(moneyPic[i],i-20,k,n);
				document.getElementById('pic-3').appendChild(moneyPic[i]);
			};
